...
export default async function handler(req, res) {
  ...
  try {
    const { wine_name, producer, vintage, region, grapes, style, currency } = req.body || {};
    if (!wine_name) return res.status(400).json({ error: "wine_name required" });

    const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });

    const currencyNote = currency && currency !== "USD" ? `\nUser's currency: ${currency} — quote market_value_estimate in this currency` : "";
    const parts = [
      `Wine: ${wine_name}`,
